-- Optional expansion/set filter for marketplace browse (sidebar + nav set search).
...
BEGIN
  ...
  cards_matching_raw AS MATERIALIZED (
    SELECT c.id
  ...
  cards_matching_text AS MATERIALIZED (
    SELECT cmr.id
    FROM cards_matching_raw cmr
    JOIN public.cards_v2 c ON c.id = cmr.id
