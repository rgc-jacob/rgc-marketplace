-- Fix statement timeouts on text search: wide OR + ILIKE prevented index use and scanned huge joins.
-- Strategy: pg_trgm GIN indexes + MATERIALIZED CTE of matching card ids (UNION of narrow predicates),
-- then join seller rows / price rows to that set. Empty/null p_query skips the IN filter.

CREATE EXTENSION IF NOT EXISTS pg_trgm;

CREATE INDEX IF NOT EXISTS idx_cards_v2_name_gin_trgm
  ON public.cards_v2 USING gin (name gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_cards_v2_rarity_gin_trgm
  ON public.cards_v2 USING gin (rarity gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_cards_v2_variant_name_gin_trgm
  ON public.cards_v2 USING gin (variant_name gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_cards_v2_game_id_gin_trgm
  ON public.cards_v2 USING gin (game_id gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_cards_v2_number_text_gin_trgm
  ON public.cards_v2 USING gin ((coalesce(number::text, '')) gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_expansions_name_gin_trgm
  ON public.expansions USING gin (name gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_expansions_code_gin_trgm
  ON public.expansions USING gin (code gin_trgm_ops);
...
AS $function$
  ...
  cards_matching_text AS MATERIALIZED (
    ...
    FROM public.cards_v2 c
    WHERE p_query IS NOT NULL AND btrim(p_query) <> ''
      AND coalesce(c.rarity, '') ILIKE '%' || p_query || '%'
    UNION
    SELECT c.id
    FROM public.cards_v2 c
    WHERE p_query IS NOT NULL AND btrim(p_query) <> ''
      AND coalesce(c.number::text, '') ILIKE '%' || p_query || '%'
    UNION
    SELECT c.id
    FROM public.cards_v2 c
    WHERE p_query IS NOT NULL AND btrim(p_query) <> ''
      AND coalesce(c.variant_name, '') ILIKE '%' || p_query || '%'
    UNION
    SELECT c.id
    FROM public.cards_v2 c
    WHERE p_query IS NOT NULL AND btrim(p_query) <> ''
      AND coalesce(c.game_id, '') ILIKE '%' || p_query || '%'
    UNION
    SELECT c.id
    FROM public.cards_v2 c
    INNER JOIN public.expansions ex ON ex.id = c.expansion_id
    WHERE p_query IS NOT NULL AND btrim(p_query) <> ''
      AND coalesce(ex.name, '') ILIKE '%' || p_query || '%'
    UNION
    SELECT c.id
    FROM public.cards_v2 c
    INNER JOIN public.expansions ex ON ex.id = c.expansion_id
    WHERE p_query IS NOT NULL AND btrim(p_query) <> ''
      AND coalesce(ex.code, '') ILIKE '%' || p_query || '%'
  ),
  sl AS (
    SELECT
    ...
    WHERE ml.status = 'active'
      AND (
        p_query IS NULL
        OR btrim(p_query) = ''
        OR ml.card_id IN (SELECT id FROM cards_matching_text)
  ...
  ref AS (
    ...
    WHERE lcp.price IS NOT NULL
      ...
      AND (
        p_query IS NULL
        OR btrim(p_query) = ''
        OR lcp.card_id IN (SELECT id FROM cards_matching_text)
      )
      AND (p_graded IS NULL OR p_graded = false)
