-- Match marketplace browse search (p_query) against card catalog fields, not only card name.
-- Covers: name, rarity, collector number, variant/set names & codes, game id, seller title/description.
  ...
  sl AS (
    ...
    WHERE ml.status = 'active'
      AND (
        ...
        OR COALESCE(c.game_id, '') ILIKE '%' || p_query || '%'
        OR COALESCE(e.name, '') ILIKE '%' || p_query || '%'
        OR COALESCE(e.code, '') ILIKE '%' || p_query || '%'
      )
  ...
  ref AS (
    ...
    WHERE lcp.price IS NOT NULL
      ...
      AND (
        ...
        OR COALESCE(c.variant_name, '') ILIKE '%' || p_query || '%'
        OR COALESCE(c.game_id, '') ILIKE '%' || p_query || '%'
        OR COALESCE(exp.name, '') ILIKE '%' || p_query || '%'
        OR COALESCE(exp.code, '') ILIKE '%' || p_query || '%'
      )
      AND (p_graded IS NULL OR p_graded = false)
