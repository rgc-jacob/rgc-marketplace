-- Empty browse was timing out: MATERIALIZED cards_matching_text still planned all UNION arms
-- (sequential scans on cards_v2) even when p_query was NULL/blank. Branch in PL/pgSQL so
-- the text-matching CTE is never referenced when there is no search string.
...
BEGIN
  ...
  cards_matching_text AS MATERIALIZED (
    ...
    SELECT c.id
    FROM public.cards_v2 c
    WHERE coalesce(c.rarity, '') ILIKE '%' || p_query || '%'
    UNION
    SELECT c.id
    FROM public.cards_v2 c
    WHERE coalesce(c.number::text, '') ILIKE '%' || p_query || '%'
    UNION
    SELECT c.id
    FROM public.cards_v2 c
    WHERE coalesce(c.variant_name, '') ILIKE '%' || p_query || '%'
    UNION
    SELECT c.id
    FROM public.cards_v2 c
    WHERE coalesce(c.game_id, '') ILIKE '%' || p_query || '%'
    UNION
    SELECT c.id
    FROM public.cards_v2 c
    INNER JOIN public.expansions ex ON ex.id = c.expansion_id
    WHERE coalesce(ex.name, '') ILIKE '%' || p_query || '%'
    UNION
    SELECT c.id
    FROM public.cards_v2 c
    INNER JOIN public.expansions ex ON ex.id = c.expansion_id
    WHERE coalesce(ex.code, '') ILIKE '%' || p_query || '%'
  ),
  sl AS (
    SELECT
      ml.id::text AS listing_key,
